...
PIXI.loader
  .add(["public/assets/images/dunjo_cover.png"])
  ...
  .on("load", (loader, resources) => {
    // Wall init
    textures.wall_up_left = new PIXI.Texture(
      resources.texture,
      new PIXI.Rectangle(43, 75, 25, 25)
    );
    sprites.wall_up_left = new PIXI.Sprite(textures.wall_up_left);

    textures.wall_up_right = new PIXI.Texture(
      resources.texture,
      new PIXI.Rectangle(94, 75, 25, 25)
    );
    sprites.wall_up_right = new PIXI.Sprite(textures.wall_up_right);

    textures.wall_bottom_left = new PIXI.Texture(
      resources.texture,
      new PIXI.Rectangle(43, 126, 25, 25)
    );
    sprites.wall_bottom_left = new PIXI.Sprite(textures.wall_bottom_left);

    textures.wall_bottom_right = new PIXI.Texture(
      resources.texture,
      new PIXI.Rectangle(94, 126, 25, 25)
    );
    sprites.wall_bottom_right = new PIXI.Sprite(textures.wall_bottom_right);

    textures.wall_right = new PIXI.Texture(
      resources.texture,
      new PIXI.Rectangle(45, 103, 25, 25)
    );
    sprites.wall_right = new PIXI.Sprite(textures.wall_right);

    // Persona init
    textures.player = new PIXI.Texture(
      resources.texture,
      new PIXI.Rectangle(128, 132, 14, 16)
    );
...
createjs.Sound.registerSound(
  "public/assets/sound/les_copains_dabord.mp3",
  "Brassens"
);
